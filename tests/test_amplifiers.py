...
def test_amplifiers(data, answer):
    assert Amplifier(data, 5).solve() == answer


@pytest.mark.skipif(
    os.getenv("PYTEST_FILTER") == "SHORT",
    reason="skipping longer tests"
)
@pytest.mark.parametrize(
    ("data", "answer"),
    [
...
def test_amplifiers_feedback(data, answer):
    assert Amplifier(data, 5).solve_feedback() == answer
